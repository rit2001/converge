...
const environmentSchema = z
  ...
  .superRefine((environment, context) => {
    if (environment.REDIS_PUBLISH_TIMEOUT_MS >= environment.OUTBOX_LEASE_MS)
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["REDIS_PUBLISH_TIMEOUT_MS"],
        message: "Publication timeout must be shorter than the outbox lease",
      });
  });
